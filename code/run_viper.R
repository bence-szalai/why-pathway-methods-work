  ...
  for (abs_type in c('_abs','')){
    ...
    for (sin_or_over in c('single','overlap')){
      ...
      for (geneset in genesets){
        print(c(dname,abs_type,sin_or_over,geneset))
        load(paste0('../results/genesets/',sin_or_over,'/rdatas/',geneset,'.rdata'))
        if (length(viper_geneset)>0){
          activities = viper(eset = gex, regulon = viper_geneset, nes = T, 
                             method = 'none' ,minsize = 1, eset.filter = F,cores = ncores)
          write.csv(activities,paste0('../results/benchmark/scores/',dname,'/',
                                      sin_or_over,'/',geneset,abs_type,'.csv'))
        }
      }
    }
  }
